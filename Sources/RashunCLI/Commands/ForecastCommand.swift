...
struct ForecastCommand: AsyncParsableCommand {
    ...
    func run() async throws {
            ...
            do {
                ...
                if source.metrics.count > 1 {
                    SourceHealthStore.shared.recordSuccess(
                        sourceName: source.name, metricId: selectedMetric.id, usage: usage)
                } else {
                ...
                let forecast = source.forecast(
                    for: selectedMetric.id, current: usage, history: history)
                ...
                if source.metrics.count > 1 {
                    SourceHealthStore.shared.recordFailure(
                        sourceName: source.name, metricId: selectedMetric.id,
                ...
                ForecastResponse(
                    ...
                    metrics: forecasts.map { forecast in
                        ForecastMetricResponse(
                            ...
                            points: forecast.forecast?.points.map {
                                ForecastPointResponse(date: $0.date, value: $0.value)
                            } ?? []
                        )
                    }
                ))
        ...
        print(
            "\(formatter.emoji("📈", fallback: "*")) \(formatter.colorize("\(source.displayName) Forecast", as: .bold))"
        )
        for forecast in forecasts {
            if let result = forecast.forecast {
                ...
                if source.metrics.count > 1 {
                    print(
                        "\(forecast.metric.title): No forecast available. Not enough usage history or no active reset window."
                    )
                } else {
                    print(
