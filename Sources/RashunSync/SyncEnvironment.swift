...
public final class SyncEnvironment {
    ...
    private init() {
        do {
            ...
            let repo = try SyncRepository(
                path: directory.appendingPathComponent("usage-sync.sqlite").path)
            repository = repo
            if try repo.allObservations().isEmpty {
                let legacy = UsageHistoryStore.shared.allHistory()
                if !legacy.isEmpty {
                    let data = try JSONEncoder().encode(legacy)
                    _ = try LegacyHistoryMigrator.migrate(
                        history: legacy, sourceData: data, repository: repo,
                        backupRoot: directory.appendingPathComponent(
                            "Backups/sync-v1", isDirectory: true),
                        registry: LegacyHistoryMigrator.defaultRegistry())
                    _ = UsageHistoryStore.shared.replaceAllHistory(
                        try repo.projectedHistory(), force: true)
                }
            }
        } catch {
            repository = nil
    ...
    @discardableResult public func record(
        providerID: String, metricID: String, usage: UsageResult, at: Date = Date()
    ) throws -> UsageObservation {
        guard let repository else { throw startupError ?? CocoaError(.fileWriteUnknown) }
        let observation = try repository.record(
            series: .init(providerID: providerID, metricID: metricID), usage: usage, at: at)
        // Compatibility consumers remain on the deterministic materialized view during v1.
        _ = UsageHistoryStore.shared.replaceAllHistory(
            try repository.projectedHistory(), force: true)
        return observation
    }
    ...
    public static func dataDirectory() -> URL {
        ...
        #elseif os(Windows)
            return URL(
                fileURLWithPath: ProcessInfo.processInfo.environment["APPDATA"]
