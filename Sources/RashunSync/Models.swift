...
public struct OriginID: Hashable, Codable, Sendable {
    ...
    public init(deviceID: UUID, epoch: UUID) {
...
public struct DeviceIdentity: Codable, Sendable, Equatable {
    ...
    public init(
        deviceID: UUID = UUID(), epoch: UUID = UUID(), displayName: String, signingPublicKey: Data
    ) {
        self.deviceID = deviceID
        self.epoch = epoch
...
public struct UsageObservation: Codable, Sendable, Identifiable, Equatable {
    ...
    public init(
        id: UUID = UUID(), origin: OriginID, originSequence: UInt64, series: UsageSeriesID,
        observedAt: Date, remaining: Double, limit: Double, resetAt: Date?, cycleStartedAt: Date?,
        status: ObservationStatus = .available, payloadHash: Data? = nil
    ) throws {
        ...
        self.payloadHash =
            payloadHash
            ?? Self.hash(
                id: id, origin: origin, sequence: originSequence, series: series,
        ...
        guard
            self.payloadHash
                == Self.hash(
                    ...
                    observedAt: observedAt, remaining: remaining, limit: limit,
                    resetAt: resetAt, cycleStartedAt: cycleStartedAt, status: status)
        else { throw SyncValidationError.hashMismatch }
    }

    public var usageResult: UsageResult {
        UsageResult(
            remaining: remaining, limit: limit, resetDate: resetAt, cycleStartDate: cycleStartedAt)
    }
    ...
    private static func hash(
        id: UUID, origin: OriginID, sequence: UInt64, series: UsageSeriesID, observedAt: Date,
        remaining: Double, limit: Double, resetAt: Date?, cycleStartedAt: Date?,
        status: ObservationStatus
    ) -> Data {
        let fields = [
            "1", id.uuidString.lowercased(), origin.deviceID.uuidString.lowercased(),
            origin.epoch.uuidString.lowercased(),
            String(sequence), series.providerID, series.metricID,
            String(observedAt.timeIntervalSince1970.bitPattern),
            String(remaining.bitPattern), String(limit.bitPattern),
            resetAt.map { String($0.timeIntervalSince1970.bitPattern) } ?? "-",
            cycleStartedAt.map { String($0.timeIntervalSince1970.bitPattern) } ?? "-",
            status.rawValue,
        ]
...
public struct OriginSummary: Codable, Sendable, Equatable {
    ...
    public let gaps: [SequenceRange]
}
public struct SequenceRange: Codable, Sendable, Equatable {
    public let from: UInt64
    public let through: UInt64
    public init(from: UInt64, through: UInt64) {
        self.from = from
