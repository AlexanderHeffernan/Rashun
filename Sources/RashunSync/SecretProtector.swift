...
public final class SecretProtector: @unchecked Sendable {
    ...
    public init(storageDirectory: URL, service: String = "com.alexanderheffernan.rashun.sync")
        throws
    {
        let material: Data
        ...
        if FileManager.default.fileExists(atPath: url.path) {
            ...
            guard (attrs[.posixPermissions] as? NSNumber)?.intValue == 0o600 else {
                throw SecretProtectionError.insecurePermissions
            }
        } else {
            #if canImport(Security)
                ...
                let query: [String: Any] = [
                    ...
                    kSecAttrAccount as String: "database-master-key",
                    kSecReturnData as String: true,
                ]
                var result: CFTypeRef?
                let status = SecItemCopyMatching(query as CFDictionary, &result)
                if status == errSecSuccess, let found = result as? Data {
                    material = found
                } else if status == errSecItemNotFound {
                    material = Self.random()
                } else {
                    throw SecretProtectionError.storageUnavailable
                }
            #else
                material = Self.random()
            #endif
        ...
        guard material.count == 32 else { throw SecretProtectionError.invalidKey }
        key = SymmetricKey(data: material)
    }
    public func seal(_ plaintext: Data) throws -> Data {
        let box = try AES.GCM.seal(plaintext, using: key)
        guard let combined = box.combined else { throw SecretProtectionError.encryptionFailed }
        return Data("RSE1".utf8) + combined
    }
    public func open(_ protected: Data) throws -> Data {
...
public enum SecretProtectionError: Error {
    case storageUnavailable, insecurePermissions, invalidKey, encryptionFailed
}
