...
public actor PairingChallengeStore {
    ...
    public func create(now: Date = Date()) -> Challenge {
        var rng = SystemRandomNumberGenerator()
        let secret = Data((0..<32).map { _ in UInt8.random(in: .min ... .max, using: &rng) })
        let id = UUID()
        states[id] = State(
            hash: Data(SHA256.hash(data: secret)), expiresAt: now.addingTimeInterval(120))
        return .init(id: id, secret: secret, expiresAt: now.addingTimeInterval(120))
    }
    public func consume(id: UUID, secret: Data, now: Date = Date()) -> Bool {
        guard var s = states[id], !s.consumed, s.expiresAt > now, s.attempts < 5 else {
            return false
        }
        s.attempts += 1
        let ok = s.hash == Data(SHA256.hash(data: secret))
        s.consumed = ok
        states[id] = s
        return ok
    }
