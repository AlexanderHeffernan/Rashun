...
- **Six sources built in** — Ships with support for **Amp** (Free, subscription agent usage, and subscription orb usage), **Claude** (5-hour session, weekly, and model-scoped weekly limits), **GitHub Copilot**, **Codex** (free weekly plus paid-plan 5-hour/weekly windows), **Cursor**, and **Gemini CLI** (with per-model metric tracking for Gemini). Enable whichever ones you use.
...
| **Claude** | Session, Weekly, Model Weekly | Reads Claude Code's OAuth token (macOS Keychain item `Claude Code-credentials`, or `~/.claude/.credentials.json`) and calls the Claude usage API that powers Claude Code's `/usage` command |
