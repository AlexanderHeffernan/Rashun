...
final class WebPushSenderTests: XCTestCase {
    func testEncryptedPayloadUsesAES128GCMRecordFormat() throws {
        ...
        let subscription = WebPushSubscription(endpoint: URL(string: "https://push.example.test/send/1")!, clientPublicKey: client.publicKey.x963Representation, authSecret: Data(repeating: 7, count: 16))
    ...
    func testVAPIDAuthorizationContainsPublicKeyAndSignedToken() throws {
        ...
        let value = try WebPushSender.vapidAuthorization(endpoint: URL(string: "https://push.example.test/send/1")!, signingKey: key, now: Date(timeIntervalSince1970: 1_000))
