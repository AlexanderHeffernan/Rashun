...
final class TrackedUsageTests: XCTestCase, @unchecked Sendable {
    ...
    func testActiveSessionSurvivesStoreReload() async { await MainActor.run { let backend = InMemoryPersistenceBackend(); let store = TrackedUsageStore(backend: backend); let label = store.createLabel(name: "Personal"); _ = store.start(label: label); store.append(observation(0, 50, origin: .start)); let reloaded = TrackedUsageStore(backend: backend); XCTAssertEqual(reloaded.activeSession?.labelNameSnapshot, "Personal"); XCTAssertEqual(reloaded.activeSession?.observations.count, 1) } }
    func testSwitchFinalizesExistingSession() async { await MainActor.run { let store = TrackedUsageStore(backend: InMemoryPersistenceBackend()); let a = store.createLabel(name: "A"); let b = store.createLabel(name: "B"); _ = store.start(label: a, at: base); _ = store.start(label: b, at: base.addingTimeInterval(60)); XCTAssertEqual(store.sessions.count, 1); XCTAssertEqual(store.sessions[0].completionState, .interrupted); XCTAssertEqual(store.activeSession?.labelNameSnapshot, "B") } }
    func testStopDiscardsSessionWithNoObservedUsage() async { await MainActor.run { let store = TrackedUsageStore(backend: InMemoryPersistenceBackend()); let label = store.createLabel(name: "Personal"); _ = store.start(label: label); store.append(observation(0, 80, origin: .start)); store.append(observation(1, 80, origin: .stop)); XCTAssertNil(store.stop()); XCTAssertTrue(store.sessions.isEmpty); XCTAssertNil(store.activeSession) } }
    func testInterleavedUnchangedMetricsAreCoalesced() async {
        await MainActor.run {
    ...
    func testBoundaryObservationsAreKeptWhenReadingIsUnchanged() async {
        await MainActor.run {
        let store = TrackedUsageStore(backend: InMemoryPersistenceBackend())
        let label = store.createLabel(name: "Personal")
        _ = store.start(label: label)
    ...
    func testRenamingLabelUpdatesCompletedAndActiveSessions() async {
        await MainActor.run {
        let store = TrackedUsageStore(backend: InMemoryPersistenceBackend())
    ...
    func testEmptyLabelRenameIsRejected() async {
        await MainActor.run {
        let store = TrackedUsageStore(backend: InMemoryPersistenceBackend())
    ...
    func testSyncSnapshotExcludesActiveSessionAndMergesCompletedSession() async {
        await MainActor.run {
    ...
    func testSyncedSessionDeletionDoesNotResurrect() async {
        await MainActor.run {
        let source = TrackedUsageStore(backend: InMemoryPersistenceBackend())
        let label = source.createLabel(name: "Client work")
        _ = source.start(label: label, at: base)
