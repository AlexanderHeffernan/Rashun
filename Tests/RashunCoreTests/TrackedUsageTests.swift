...
final class TrackedUsageTests: XCTestCase {
    ...
    func testSwitchFinalizesExistingSession() { let store = TrackedUsageStore(backend: InMemoryPersistenceBackend()); let a = store.createLabel(name: "A"); let b = store.createLabel(name: "B"); _ = store.start(label: a, at: base); _ = store.start(label: b, at: base.addingTimeInterval(60)); XCTAssertEqual(store.sessions.count, 1); XCTAssertEqual(store.sessions[0].completionState, .interrupted); XCTAssertEqual(store.activeSession?.labelNameSnapshot, "B") }
    func testStopDiscardsSessionWithNoObservedUsage() { let store = TrackedUsageStore(backend: InMemoryPersistenceBackend()); let label = store.createLabel(name: "Personal"); _ = store.start(label: label); store.append(observation(0, 80, origin: .start)); store.append(observation(1, 80, origin: .stop)); XCTAssertNil(store.stop()); XCTAssertTrue(store.sessions.isEmpty); XCTAssertNil(store.activeSession) }
    func testInterleavedUnchangedMetricsAreCoalesced() {
    ...
    func testBoundaryObservationsAreKeptWhenReadingIsUnchanged() {
        let store = TrackedUsageStore(backend: InMemoryPersistenceBackend())
        let label = store.createLabel(name: "Personal")
        _ = store.start(label: label)
    ...
    func testRenamingLabelUpdatesCompletedAndActiveSessions() {
        let store = TrackedUsageStore(backend: InMemoryPersistenceBackend())
    ...
    func testEmptyLabelRenameIsRejected() {
        let store = TrackedUsageStore(backend: InMemoryPersistenceBackend())
    ...
    func testSyncSnapshotExcludesActiveSessionAndMergesCompletedSession() {
    ...
    func testSyncedSessionDeletionDoesNotResurrect() {
        let source = TrackedUsageStore(backend: InMemoryPersistenceBackend())
        let label = source.createLabel(name: "Client work")
        _ = source.start(label: label, at: base)
        source.append(observation(0, 80, origin: .start))
